...
public class CustomSwipeLayout extends FrameLayout {
    ...
    private LinkedHashMap<DragEdge, View> mDragEdges = new LinkedHashMap<CustomSwipeLayout.DragEdge, View>();
    ...
    protected boolean isViewTotallyFirstShowed(View child, Rect relativePosition, CustomSwipeLayout.DragEdge edge, int surfaceLeft,
        ...
        if (getShowMode() == CustomSwipeLayout.ShowMode.LayDown) {
            if ((edge == CustomSwipeLayout.DragEdge.Right && surfaceRight <= childLeft)
                    ...
                    || (edge == CustomSwipeLayout.DragEdge.Bottom && surfaceBottom <= childTop))
                r = true;
            ...
            if ((edge == CustomSwipeLayout.DragEdge.Right && childRight <= getWidth())
                    ...
                    || (edge == CustomSwipeLayout.DragEdge.Bottom && childBottom <= getHeight()))
                r = true;
        }
        return r;
    }
    ...
    public void addOnLayoutListener(CustomSwipeLayout.OnLayout l) {
        if (mOnLayoutListeners == null)
            mOnLayoutListeners = new ArrayList<CustomSwipeLayout.OnLayout>();
    ...
    public void onViewRemoved(View child) {
        for (Map.Entry<CustomSwipeLayout.DragEdge, View> entry : new HashMap<DragEdge, View>(mDragEdges).entrySet()) {
